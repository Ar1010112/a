...
export default function EnhancedGrid() {
  ...
  return (
    <section className="py-20 relative overflow-hidden">
      {/* Background Effects */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#07D348]/5 to-transparent rounded-3xl -z-10"></div>
      ...
      <div ref={gridRef} className="grid gap-8 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 relative">
        {filteredItems.map((item, index) => (
          <motion.div
            ...
            className="grid-card group relative overflow-hidden rounded-3xl border-2 border-[#07D348]/20 bg-gradient-to-br from-zinc-900/80 to-zinc-900/40 backdrop-blur-xl transition-all duration-500 hover:border-[#07D348]/60 hover:shadow-2xl hover:shadow-[#07D348]/20"
            onMouseEnter={() => setHoveredItem(item.id)}
            onMouseLeave={() => setHoveredItem(null)}
            style={{ transformStyle: 'preserve-3d' }}
          >
            {/* Background Gradient Overlay */}
            <div className="absolute inset-0 bg-gradient-to-br from-[#07D348]/10 via-transparent to-[#24fe41]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
              {[...Array(6)].map((_, i) => (
                <motion.div
                  key={i}
                  className="absolute w-1 h-1 bg-[#07D348] rounded-full opacity-0 group-hover:opacity-60"
                  style={{
            ...
            <div className="relative h-48 overflow-hidden rounded-t-3xl">
              ...
              {/* Stats Badge */}
              <motion.div
                className="absolute top-4 right-4 bg-black/80 backdrop-blur-sm rounded-xl px-3 py-2 border border-[#07D348]/30"
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: hoveredItem === item.id ? 1 : 0.8, scale: hoveredItem === item.id ? 1 : 0.8 }}
              >
                <div className="text-[#07D348] font-bold text-lg">{item.stats?.value}</div>
                <div className="text-zinc-300 text-xs">{item.stats?.label}</div>
              </motion.div>

              {/* Icon Overlay */}
              <motion.div
                className="absolute bottom-4 left-4 p-3 bg-black/80 backdrop-blur-sm rounded-xl border border-[#07D348]/30"
            ...
            <div className="p-8 relative z-10">
              <motion.h3
                className="text-2xl font-bold text-white mb-4 group-hover:text-[#07D348] transition-colors duration-300"
              ...
              <div className="flex items-center justify-between">
                <span className="px-3 py-1 bg-[#07D348]/20 text-[#07D348] rounded-full text-sm font-medium border border-[#07D348]/30">
                  {item.category}
                </span>
                
                <motion.button
                  className="flex items-center gap-2 text-[#07D348] hover:text-[#24fe41] transition-colors group/btn"
                  whileHover={{ x: 5 }}
                >
            ...
            <div className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
              <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-[#07D348]/20 to-[#24fe41]/20 blur-xl" />
            </div>
          </motion.div>
        ))}
      </div>
      ...
      >
        <motion.button
          className="w-16 h-16 bg-gradient-to-r from-[#07D348] to-[#24fe41] rounded-full shadow-2xl shadow-[#07D348]/40 flex items-center justify-center text-white"
